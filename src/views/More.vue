<template>
  <div class="box">
    ...
    <ul class="Rc">
      <p>为你推荐</p>
      <li
        @click="cKK(item.designTemplateId)"
        v-for="item in picmore"
        :key="item.designFirstKindId"
      >
...
export default {
  ...
  activated() {
    ...
    this.$store.commit("setpath", this.$route.fullPath.split("/")[1]);
  },
  methods: {
    async getcontents() {
      let res = await fetch(
  ...
  watch: {
    ...
    //   if(oldValue.path!=newValue.path&&this.$store.state.path=='more'){
    //         this.imgss = this.$route.params.id;
    //         this.getcontents();
    //         console.log('变化');
    //   }
    ...
    $route: {
      handler() {
        if (this.$route.fullPath.split('/')[1]=='more'&&this.$store.state.path=='more'){
          this.imgss = this.$route.params.id;
          this.getcontents();
        }
      },
